...
public class ValidationHelper {
    ...
    public void validateModificationBackendMultiTPS(int numRecords, String travelStatusName, String securityValue, String arrivalDate, String departuredate,
        ...
        do {
            ...
            // TestReporter.softAssertEquals(rs.getValue("TPS_EXTNL_REF_TYP_NM"), extRefTypes.get(mapKey), "Verify that the external ref type name [" +
            // rs.getValue("TPS_EXTNL_REF_TYP_NM") + "] is that which is expected [" + extRefTypes.get(mapKey) + "].");
            // TestReporter.softAssertEquals(rs.getValue("TPS_EXTNL_REF_VL"), extRefs.get(mapKey), "Verify that the external ref value [" +
            // rs.getValue("TPS_EXTNL_REF_VL") + "] is that which is expected [" + extRefs.get(mapKey) + "].");
            rs.moveNext();
            counter++;
        } while (rs.hasNext());
        TestReporter.assertAll();
    ...
    public void validateModificationBackend(int numRecords, String travelStatusName, String securityValue, Map<String, String> arrivalDates, Map<String, String> departureDates,
        ...
        do {
            ...
            if (rs.getValue("TC_GRP_TYP_NM").equalsIgnoreCase("SHOWDINING")) {
                ...
                // TestReporter.softAssertEquals(rs.getValue("TPS_EXTNL_REF_TYP_NM"), "NULL", "Verify that the external ref type name [" +
                // rs.getValue("TPS_EXTNL_REF_TYP_NM") + "] is that which is expected [NULL].");
                // TestReporter.softAssertEquals(rs.getValue("TPS_EXTNL_REF_VL"), "NULL", "Verify that the external ref value [" +
                // rs.getValue("TPS_EXTNL_REF_VL") + "] is that which is expected [NULL].");
                ...
                // TestReporter.softAssertEquals(rs.getValue("TPS_EXTNL_REF_TYP_NM"), extRefType, "Verify that the external ref type name [" +
                // rs.getValue("TPS_EXTNL_REF_TYP_NM") + "] is that which is expected [" + extRefType + "].");
                // TestReporter.softAssertEquals(rs.getValue("TPS_EXTNL_REF_VL"), extRefValue, "Verify that the external ref value [" +
                // rs.getValue("TPS_EXTNL_REF_VL") + "] is that which is expected [" + extRefValue + "].");
            }
            rs.moveNext();
        } while (rs.hasNext());
        TestReporter.assertAll();
    ...
    public void validateTPV3(String tpId, String status, String arrivalDate, String departureDate, String tpPartyId, Guest guest, int accommCount, int packageCount, String ada, String groupCode, String facilityId) {
        ...
        TestReporter.softAssertEquals(rs.getValue("SLS_ORD_STS_NM"), status, "Verify that the sales order status [" + rs.getValue("SLS_ORD_STS_NM") + "] is that which is expected [" + status + "].");
    ...
    public void validateTPV3(String tpId, Map<String, String> status, String arrivalDate, String departureDate, String tpPartyId, Guest guest, int accommCount, int packageCount, String ada, String groupCode, String facilityId) {
        TestReporter.logStep("Validating TPV3 record creation");
        String sql = "select a.* "
                + "from sales_tp.tp a "
                + "join sales_tp.tp_pty b on a.tp_id = b.tp_id "
        ...
        do {
            ...
            TestReporter.softAssertEquals(rs.getValue("SLS_ORD_ITEM_STRT_DTS").split(" ")[0], arrivalDate, "Verify that the sales order item start date [" + rs.getValue("SLS_ORD_ITEM_STRT_DTS").split(" ")[0] + "] is that which is expected [" + arrivalDate + "].");
            TestReporter.softAssertEquals(rs.getValue("SLS_ORD_ITEM_END_DTS").split(" ")[0], departureDate, "Verify that the sales order item end date [" + rs.getValue("SLS_ORD_ITEM_END_DTS").split(" ")[0] + "] is that which is expected [" + departureDate + "].");
            TestReporter.softAssertTrue(status.containsKey(rs.getValue("SLS_ORD_ITEM_STS_NM")), "Verify that the sales order status [" + rs.getValue("SLS_ORD_ITEM_STS_NM") + "] is contained in the map of expected value [" + status + "].");
            TestReporter.softAssertEquals(rs.getValue("FAC_ID"), facilityId, "Verify that the sales order item facility ID [" + rs.getValue("FAC_ID") + "] is that which is expected [" + facilityId + "].");
            TestReporter.softAssertEquals(rs.getValue("GRP_CD"), groupCode, "Verify that the sales order item group code [" + rs.getValue("GRP_CD") + "] is that which is expected [" + groupCode + "].");
            TestReporter.softAssertEquals(rs.getValue("SPCL_NEED_REQ_IN"), ada, "Verify that the sales order item ADA flag [" + rs.getValue("SPCL_NEED_REQ_IN") + "] is that which is expected [" + ada + "].");
            rs.moveNext();
    ...
    public void validateTPV3(String tpId, String status, String arrivalDate, String departureDate, Guest guest, int accommCount, int packageCount, String ada, String groupCode, Map<String, String> facilityIds, Map<Integer, Guest> additionalGuests) {
        TestReporter.logStep("Validating TPV3 record creation");
        String sql = "select a.* "
                + "from sales_tp.tp a "
                + "join sales_tp.tp_pty b on a.tp_id = b.tp_id "
        ...
        do {
            ...
            TestReporter.softAssertEquals(rs.getValue("SLS_ORD_ITEM_END_DTS").split(" ")[0], departureDate, "Verify that the sales order item end date [" + rs.getValue("SLS_ORD_ITEM_END_DTS").split(" ")[0] + "] is that which is expected [" + departureDate + "].");
            TestReporter.softAssertEquals(rs.getValue("SLS_ORD_ITEM_STS_NM"), status, "Verify that the sales order item status [" + rs.getValue("SLS_ORD_ITEM_STS_NM") + "] is that which is expected [" + status + "].");
            TestReporter.softAssertTrue(facilityIds.containsKey(rs.getValue("FAC_ID")), "Verify that the sales order item facility ID [" + rs.getValue("FAC_ID") + "] is contained in the map of expected values [" + facilityIds + "].");
            TestReporter.softAssertEquals(rs.getValue("GRP_CD"), groupCode, "Verify that the sales order item group code [" + rs.getValue("GRP_CD") + "] is that which is expected [" + groupCode + "].");
            TestReporter.softAssertEquals(rs.getValue("SPCL_NEED_REQ_IN"), ada, "Verify that the sales order item ADA flag [" + rs.getValue("SPCL_NEED_REQ_IN") + "] is that which is expected [" + ada + "].");
            rs.moveNext();
        } while (rs.hasNext());
    ...
    public void verifyTravelAgency(AccommodationBaseTest base) {
        ...
        TestReporter.log("Validate travel agency");
        sql = "SELECT e.TRVL_AGT_ID "
                + "FROM  FOLIO.EXTNL_REF a "
                + "INNER JOIN FOLIO.CHRG_GRP_EXTNL_REF b ON a.EXTNL_REF_ID = b.EXTNL_REF_ID "
                + "INNER JOIN FOLIO.NODE_CHRG_GRP c ON b.CHRG_GRP_ID = c.ROOT_CHRG_GRP_ID "
        ...
        if (!(isValid(base.getIsLibgoBooking()) && (base.getIsLibgoBooking() == true)) &&
                !(isValid(base.isWdtcBooking()) && (base.isWdtcBooking() == true))) {
            sql2 = "select * "
                    + "from folio.trvl_agcy a "
                    + "where a.TRVL_AGCY_ID = " + rs.getValue("TRVL_AGT_ID");
            rs2 = new Recordset(db.getResultSet(sql2));
