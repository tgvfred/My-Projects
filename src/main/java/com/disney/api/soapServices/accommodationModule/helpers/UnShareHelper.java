...
public class UnShareHelper {
    ...
    public UnShareHelper(String environment) {
    ...
    public void validateMultipleOwnerIds(String firstTcgId, String secondTcgId) {
        TestReporter.logStep("Verify assignment owner id changes for the second tcg");
        ...
        do {
            TestReporter.softAssertTrue(assignFirstOwnerId != assignSecondOwnerId, "Verify the assignment owner Ids for each TCG [" + assignFirstOwnerId + "] do not match [" + assignSecondOwnerId + "].");
            rs.moveNext();
            TestReporter.assertAll();
        } while (rs.hasNext());
