...
public class Dreams_AccommodationQueries extends Dreams {
    ...
    public static String getTpPartyGuestInfoByTpId_NoMembership(String tpId) {
        return "select b.IDVL_FST_NM, b.IDVL_MID_NM, b.IDVL_LST_NM, b.DVC_MBR_TYP_NM, e.ADDR_LN_1_TX, e.CITY_NM, e.RGN_CD, e.PSTL_CD, f.TXN_PTY_EML_ADDR_TX, g.PHN_NB "
                + "from res_mgmt.tp_pty a, guest.txn_idvl_pty b, guest.TXN_PTY_LCTR d , guest.TXN_PTY_ADDR_LCTR e, guest.TXN_PTY_EML_LCTR f, guest.TXN_PTY_PHN_LCTR g "
                + "where a.tp_id = 471637109211 "
                + "and b.TXN_IDVL_PTY_ID = a.TXN_PTY_ID "
                + "and a.TXN_PTY_ID = d.TXN_PTY_ID "
                + "and e.TXN_PTY_ADDR_LCTR_ID = ( "
                + "        select TXN_PTY_ADDR_LCTR_ID"
                + "        from res_mgmt.tp_pty a "
                + "        left outer join guest.txn_idvl_pty b on b.TXN_IDVL_PTY_ID = a.TXN_PTY_ID "
                + "        left outer join guest.TXN_IDVL_PTY_MBSHP c on b.TXN_IDVL_PTY_ID = c.TXN_IDVL_PTY_ID "
                + "        left outer join guest.TXN_PTY_LCTR d on a.TXN_PTY_ID = d.TXN_PTY_ID "
                + "        left outer join guest.TXN_PTY_ADDR_LCTR e on d.TXN_PTY_LCTR_ID = e.TXN_PTY_ADDR_LCTR_ID "
                + "        where a.tp_id = " + tpId + " "
                + "        and TXN_PTY_ADDR_LCTR_ID is not null) "
                + "and f.TXN_PTY_EML_LCTR_ID = ( "
                + "        select TXN_PTY_EML_LCTR_ID "
                + "        from res_mgmt.tp_pty a "
                + "        left outer join guest.txn_idvl_pty b on b.TXN_IDVL_PTY_ID = a.TXN_PTY_ID "
                + "        left outer join guest.TXN_IDVL_PTY_MBSHP c on b.TXN_IDVL_PTY_ID = c.TXN_IDVL_PTY_ID "
                + "        left outer join guest.TXN_PTY_LCTR d on a.TXN_PTY_ID = d.TXN_PTY_ID "
                + "        left outer join guest.TXN_PTY_EML_LCTR e on d.TXN_PTY_LCTR_ID = e.TXN_PTY_EML_LCTR_ID "
                + "        where a.tp_id = " + tpId + " "
                + "        and TXN_PTY_EML_LCTR_ID is not null) "
                + "and g.TXN_PTY_PHN_LCTR_ID = ( "
                + "        select TXN_PTY_PHN_LCTR_ID "
                + "        from res_mgmt.tp_pty a "
                + "        left outer join guest.txn_idvl_pty b on b.TXN_IDVL_PTY_ID = a.TXN_PTY_ID "
                + "        left outer join guest.TXN_IDVL_PTY_MBSHP c on b.TXN_IDVL_PTY_ID = c.TXN_IDVL_PTY_ID "
                + "        left outer join guest.TXN_PTY_LCTR d on a.TXN_PTY_ID = d.TXN_PTY_ID "
                + "        left outer join guest.TXN_PTY_PHN_LCTR e on d.TXN_PTY_LCTR_ID = e.TXN_PTY_PHN_LCTR_ID "
                + "        where a.tp_id = " + tpId + " "
                + "        and TXN_PTY_PHN_LCTR_ID is not null)";
    }
