...
public class TestArrived {
	...
	public void testArrived(String Scenario, String SignInLocation,
		...
		if (ServiceStyle.equalsIgnoreCase("TableService")) {
			...
			TestReporter.logStep("Update Reservation to Arrived");
			arrived.set(new Arrived(environment.get(), "Main"));
			arrived.get().setCommunicationChannel(communicationsChannel.get());
			arrived.get().setSalesChannel(salesChannel.get());
			arrived.get().setReservationNumber(TPS_ID.get());
			arrived.get().sendRequest();
			TestReporter.logAPI(!arrived.get().getResponseStatusCode().equals("200"), "An error occurred in updating the reservation to 'Arrived'.", arrived.get());
			try{TestReporter.assertEquals(arrived.get().getArrivalStatus(), "SUCCESS", "The reservation status was not [SUCCESS] as expected, but was found to be ["+arrived.get().getArrivalStatus()+"] instead.");}
			catch(XPathNotFoundException e){throw new XPathNotFoundException("An xpath exception occurred extracting the arrival status from the response. \nResponse:\n" + arrived.get().getResponse());}
		}
	}
	private void book(String ServiceStyle, String BookScenario, String SignInLocation, String ComponentType, String ComponentId,
		...
		TestReporter.logAPI(!eventDiningBook.get().getResponseStatusCode().equals("200"), "An error occurred during booking.", arrived.get());
