...
public class Test_ReverseAndPostCharges extends BaseTest{
	...
	public void testReverseAndPostChargesWithAddon(){
		...
		TestReporter.logStep("Validate Charge info for Charge ID [" + chargeId + "] in Folio.Chrg table was DELETED");
		Recordset rsOldChargeId = new Recordset(db.getResultSet(Dreams.getChargeInfo(chargeId)));
		TestReporter.assertTrue(rsOldChargeId.getRowCount() == 0 , "Validate no Charge items returned in query [" + Dreams.getChargeInfo(chargeId) +"]");
		...
		TestReporter.assertTrue(rsNewChargeId.getRowCount() > 0 , "Validate Charge items are returned in query [" + Dreams.getChargeInfo(chargeId) +"]. Records found [" +rsNewChargeId.getRowCount() +"]");
	...
	public void testReverseAndPostChargesWithNoAddon(){
		...
		rs.print();
		for(rs.moveFirst() ; rs.hasNext() ; rs.moveNext()){
			if(rs.getValue("PROD_TYP_NM").toLowerCase().contains("package")){
				tcId = rs.getValue("TC_ID");
				tcgId = rs.getValue("TC_GRP_NB");
				productTypeName = rs.getValue("PROD_TYP_NM");
			}
