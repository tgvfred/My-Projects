...
public class TestAutoArrived {
	 ...
	 */
	protected ThreadLocal<AutoArrived> autoArrived = new ThreadLocal<AutoArrived>();
	...
	public void testAutoArrived(String Scenario, String SignInLocation,
		...
		if(iteration < maxIterations){
			if (ServiceStyle.equalsIgnoreCase("TableService")) {
				...
				TestReporter.logStep("Update Reservation to 'Auto Arrived'");
				autoArrived.set(new AutoArrived(environment.get(), "Main"));
				autoArrived.get().setTpsId(TPS_ID.get());
				autoArrived.get().sendRequest();
				TestReporter.logAPI(!autoArrived.get().getResponseStatusCode().equals("200"), "An error occurred while applying Auto Arrived.", autoArrived.get());
				
				retreiveReservation();
				TestReporter.assertEquals(eventDiningRetrieve.get().getStatus(), "Auto Arrived", "The actual status ["+eventDiningRetrieve.get().getStatus()+"] did not match the expected status [Auto Arrived].");
			}
		}
	}
