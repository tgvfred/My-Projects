...
public class TestArrived {
	...
	public void testArrived(String Scenario, String SignInLocation,
		...
		if (ServiceStyle.equalsIgnoreCase("TableService")) {
			...
			TestReporter.logStep("Update Table Service Reservation to [Arrived]");
			try{arrived.set(new Arrived(environment.get(), "Main"));}
			...
			try{
				TestReporter.assertEquals(arrived.get().getArrivalStatus(), "SUCCESS", "The reservation status was not [SUCCESS] as expected, but was found to be ["+arrived.get().getArrivalStatus()+"] instead.");
			}catch(XPathNotFoundException e){
				throw new XPathNotFoundException("An xpath exception occurred extracting the arrival status from the response. \nResponse:\n" + arrived.get().getResponse());
			}
		}
	}
}
