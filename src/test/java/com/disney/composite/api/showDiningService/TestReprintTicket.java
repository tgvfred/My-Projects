...
public class TestReprintTicket {
	...
	public void testReprintTicket(String Scenario, String SignInLocation,
		...
		if (RunTest.equalsIgnoreCase("true")) {
			if(!ServiceStyle.equalsIgnoreCase("TableService")){
				...
				TestReporter.logStep("Print Ticket");
				print.set(new PrintTicket(environment.get(), "Main"));
				print.get().setReservationnumber(TPS_ID.get());
				print.get().sendRequest();
				TestReporter.logAPI(!print.get().getResponseStatusCode().equals("200"), "An error occurred while printing a ticket.", print.get());
				
				TestReporter.logStep("RePrint Ticket");
				reprint.set(new ReprintTicket(environment.get(), "Main"));
				reprint.get().setReservationNumber(TPS_ID.get());
				reprint.get().setReprintReasonId("1");
				reprint.get().sendRequest();
				TestReporter.logAPI(!reprint.get().getResponseStatusCode().equals("200"), "An error occurred while reprinting a ticket.", reprint.get());
				TestReporter.assertEquals(reprint.get().getStatus(), "SUCCESS", "The status ["+reprint.get().getStatus()+"] was not 'SUCCESS' as expected.");
			}
		}
	}
}