	...
	public void testArrived(String Scenario, String SignInLocation,
		...
		if (RunTest.equalsIgnoreCase("true")) {
			if(!ServiceStyle.equalsIgnoreCase("TableService")){
				...
				TestReporter.logStep("Update Show Dining Reservation to [Arrived]");
				arrived.set(new Arrived(environment.get(), "BaseRequest"));
				arrived.get().setCommunicationsChannel(showDiningBook.get().getCommunicationsChannel());
				arrived.get().setReservatinoNumber(TPS_ID.get());
				arrived.get().setSalesChannel(showDiningBook.get().getSalesChannel());
				arrived.get().sendRequest();
				TestReporter.logAPI(!arrived.get().getResponseStatusCode().equals("200"), "An error occurred while updating the reservation to 'Arrived'.", arrived.get());
				TestReporter.assertEquals(arrived.get().getResponseStatus(), "SUCCESS", "The response status ["+arrived.get().getResponseStatus()+"]was not 'SUCCESS' as expected.");
			}
		}
	}
}
