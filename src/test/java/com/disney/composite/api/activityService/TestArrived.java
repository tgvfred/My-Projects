...
public class TestArrived {
	...
	public Object[][] scenarios() {
		...
		try {return datatable.get().getTestScenariosByApp(application.get(),	"CreateViewCARTermIDValid_API");}
	...
	public void testArrived(String Scenario, String Role,
	...
	private void updateToArrived(){
		...
		TestReporter.logAPI(!arrived.get().getResponseStatusCode().equals("200"), "An error occurred updating the reservation to 'Arrived.'", arrived.get());
		TestReporter.assertEquals(arrived.get().getArrivalStatus(), "SUCCESS", "The arrive status ["+arrived.get().getArrivalStatus()+"] did not match 'SUCCESS' as expected.");
	}
