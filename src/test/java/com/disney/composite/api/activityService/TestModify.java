...
public class TestModify {
	...
	protected ThreadLocal<String> PartyId = new ThreadLocal<String>();
	...
	protected ThreadLocal<Modify> modify = new ThreadLocal<Modify>();
	/*
	...
	private void modifyRes(){
		TestReporter.logStep("Modify Reservation.");
		modify.set(new Modify(environment.get(), "Main"));
		modify.get().setCommunicationChannel(CommunicationsChannel.get());
		modify.get().setFacilityId(FacilityId.get());
		modify.get().setPartyId(PartyId.get());
		modify.get().setProductId(prodId.get().getFirstProductId());
		modify.get().setReservationNumber(TPS_ID.get());
		modify.get().setSalesChannel(SalesChannel.get());
		modify.get().setSourceAccountingCenter(SourceAccountingCenter.get());
		modify.get().setTravelPlanId(TP_ID.get());
		Sleeper.sleep(Randomness.randomNumberBetween(1, 10) * 1000);
		modify.get().sendRequest();
		if(modify.get().getResponse().contains("Row was updated or deleted by another transaction")){
			Sleeper.sleep(Randomness.randomNumberBetween(1, 10) * 1000);
			modify.get().sendRequest();			
		}
		TestReporter.logAPI(!modify.get().getResponseStatusCode().equals("200"), "An error occurred during modification.", modify.get());
		TestReporter.assertEquals(modify.get().getStatus(), "SUCCESS", "The status ["+modify.get().getStatus()+"] was not 'SUCCESS' as expected.");
	}
	...
	private void cancelActivityEvent(boolean testForFailure) {
		TestReporter.log("Cancel Reservation ["+TPS_ID.get()+"]");
	...
	private void bookActivity(int booking) {
		TestReporter.log("Book Activity Event");
		...
		TestReporter.log("TP ID: " + TP_ID.get());
		TestReporter.log("TPS ID: " + TPS_ID.get());
	...
	private void retreiveReservation(int booking) {
		...
		PartyId.set(retrieve.get().getPartyId());
	}
}