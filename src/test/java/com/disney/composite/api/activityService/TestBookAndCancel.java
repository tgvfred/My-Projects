...
public class TestBookAndCancel {
	...
	@Test(dependsOnMethods = {"testBook"}, groups = {"api", "regression", "activity", "activityService"}, alwaysRun = true)
	...
	public void testBook(String Scenario, String Role,
	...
	private void cancelActivityEvent(boolean testForFailure) {
		TestReporter.logStep("Cancel Reservation ["+tpsid+"].");
		cancel.set(new Cancel(environment.get(), "CancelDiningEvent"));
		cancel.get().setReservationNumber(tpsid);
	...
	private void bookActivity(int booking) {
		TestReporter.logStep("Book Activity Event");
		...
		RetrieveProductIdByFacilityId prodId = new RetrieveProductIdByFacilityId(environment.get(), arrFacilityId.get()[booking], "ActivityProduct", true, true);
		book.get().setActivityProductId(prodId.getFirstProductId());
