...
public class TestSearchByGuest {
	...
	protected ThreadLocal<SearchByGuest> search = new ThreadLocal<SearchByGuest>();
	/*
	 * Reservation fields
	 */
	...
	public void testSearchByGuest(String Scenario, String SignInLocation,
		...
		if(iteration < maxIterations){
			if (ServiceStyle.equalsIgnoreCase("TableService")) {
				...
				search.set(new SearchByGuest(environment.get(), "Main"));
				search.get().setReservationNumber(TPS_ID.get());
				...
				TestReporter.assertEquals(search.get().getReservationNumber(), TPS_ID.get(), "The actual reservation number ["+search.get().getReservationNumber()+"] did not match the expected reservation number ["+TPS_ID.get()+"].");
				TestReporter.assertEquals(search.get().getFacilityId(), facilityId.get(), "The actual facility ID ["+search.get().getFacilityId()+"] did not match the expected facility ID ["+facilityId.get()+"].");
			}
		}
	}
}
