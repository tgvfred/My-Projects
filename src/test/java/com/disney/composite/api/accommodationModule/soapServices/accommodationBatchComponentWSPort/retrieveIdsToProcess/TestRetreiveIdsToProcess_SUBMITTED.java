...
public class TestRetreiveIdsToProcess_SUBMITTED extends AccommodationBaseTest {
	...
	private void validateProcessType(String processType, String processId, RetreiveIdsToProcess retreiveIds){
        ...
        TestReporter.softAssertEquals(processId, processId, "Verify the process Id of [" + processId + "] "
                + "brought back in the RetreiveIdsToProcess response matches the processDataIdList value [" + retreiveIds.getProcessDataIdList() + "] , and the process type [" + processType + "]found in GRP_RES_PROC_RUN database using the process Id [ "
                + processId + "]");
