...
public class Checkout_roomOnly_DVC extends BookDVCCashHelper {
    ...
    private void validateCheckoutReason() {
        ...
        Recordset rs = new Recordset(db.getResultSet(sql));
        if (rs.getRowCount() == 0) {
            throw new SQLValidationException("No charges found for tp ID [ " + getFirstBooking().getTravelComponentGroupingId() + " ]", sql);
        } else {
            do {
    ...
    public void validateCharges() {
        ...
        do {
            ...
            TestReporter.softAssertEquals(rs.getValue("CHRG_ACTV_IN"), "N", "Verify that the charge active indicator [" + rs.getValue("CHRG_ACTV_IN") + "] is that which is expected [Y].");
