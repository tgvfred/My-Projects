...
public class TestRetreiveIdsToProcess_FAILED  extends AccommodationBaseTest {
	...
	private void validateProcessType(String processType, String processId, RetreiveIdsToProcess retreiveIds){
        ...
        TestReporter.softAssertEquals(processType, pType, "Verify the process type of [" + processType + "] "
                + "matches the process type brought back from the RetreiveIdsToProcess response [" + pType + "] found in GRP_RES_PROC_RUN_STS_NM database using the process Id "
                + processId);
