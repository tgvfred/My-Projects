...
public class TestSearchResortReservationsByGuest_roomOnly_reservationNumberOnly extends AccommodationBaseTest {
        ...
        if (numberOfResortReservations > 150) {
            ...
            for (int index = 1; index <= numberOfResortReservations; index++) {
                ...
                if (reservationStatus.equals("Booked")) {
                    ...
                    TestReporter.assertTrue(reservationStatus.equals("Booked"), "The Reservation at index [" + index + "] has a Reservation status of " + reservationStatus + ". The number of Resort Reservations are [" + numberOfResortReservations + "].");

                    TestReporter.assertEquals(searchRRByGuest.getReservationNumber(), getBook().getTravelPlanSegmentId(), "The Reservation number from the response [" + searchRRByGuest.getReservationNumber() + "] matches the Reservation Number in the request [" + getBook().getTravelPlanSegmentId() + "].");
                    ...
                    TestReporter.assertTrue(reservationStatus.equals("CHECKED-IN"), "The Reservation at index [" + index + "] is " + reservationStatus + ". The number of Resort Reservations are [" + numberOfResortReservations + "].");

                } else if (reservationStatus.equals("CHECKED-OUT")) {
                    TestReporter.assertTrue(reservationStatus.equals("CHECKED-OUT"), "The Reservation at index [" + index + "] is " + reservationStatus + ". The number of Resort Reservations are [" + numberOfResortReservations + "].");
                }
