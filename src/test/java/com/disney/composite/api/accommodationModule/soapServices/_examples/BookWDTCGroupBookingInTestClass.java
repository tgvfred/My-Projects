...
public class BookWDTCGroupBookingInTestClass extends AccommodationBaseTest {
    ...
    private void validations() {
        ...
        int charges = getNights() * 13;
        helper.verifyChargeDetail(charges, getBook().getTravelPlanId());
        helper.verifyNumberOfChargesByStatus("UnEarned", 9, getBook().getTravelPlanId());
        ...
        do {
            ...
            TestReporter.softAssertEquals(tpsRs.getValue("TPS_SECUR_VL"), "NTP", "Verify that the TPS security value [" + tpsRs.getValue("TPS_SECUR_VL") + "] is that which is expected [NTP].");
            TestReporter.softAssertEquals(tpsRs.getValue("TPS_CNCL_DTS").split(" ")[0], "NULL", "Verify that the cancel DTS [" + tpsRs.getValue("TPS_CNCL_DTS").split(" ")[0] + "] is that which is expected [NULL].");
            TestReporter.softAssertEquals(tpsRs.getValue("TPS_CNCL_NB"), "NULL", "Verify that the cancel number [" + tpsRs.getValue("TPS_CNCL_NB") + "] is a numerical value as expected.");
        ...
        do {
            ...
            types.put("ComponentTravelComponent", "ComponentTravelComponent");
            types.put("PackageTravelComponent", "PackageTravelComponent");

            Map<String, String> facilities = new LinkedHashMap<>();
            facilities.put(getFacilityId(), getFacilityId());
            facilities.put("NULL", "NULL");
            ...
            if (tcRs.getValue("PROD_TYP_NM").equals("AccommodationProduct")) {
                ...
                // TestReporter.softAssertEquals(tcRs.getValue("TC_CHRG_IN"), "Y", "Verify that the TC charge indicator [" + tcRs.getValue("TC_CHRG_IN") + "] is
                // that which is expected [Y].");
                ...
                TestReporter.softAssertTrue(Regex.match("[0-9]+", tcRs.getValue("TC_ID")), "Verify that the non-accommodation TC ID [" + tcRs.getValue("TC_ID") + "] is a numerical as expected.");
                TestReporter.softAssertTrue(types.containsKey(tcRs.getValue("TC_TYP_NM")), "Verify that the TC type name [" + tcRs.getValue("TC_TYP_NM") + "] is contained in a map of expected values [" + types + "].");
                TestReporter.softAssertTrue(facilities.containsKey(tcRs.getValue("FAC_ID")), "Verify that the facility ID [" + tcRs.getValue("FAC_ID") + "] is contained in the map of expected values[" + facilities + "].");
                // TestReporter.softAssertEquals(tcRs.getValue("TC_CHRG_IN"), "N", "Verify that the TC charge indicator [" + tcRs.getValue("TC_CHRG_IN") + "] is
                // that which is expected [N].");
            ...
            TestReporter.softAssertEquals(tcRs.getValue("TC_CNCL_DTS"), "NULL", "Verify that the cancel date [" + tcRs.getValue("TC_CNCL_DTS") + "] is that which is expected [NULL].");
            ...
            TestReporter.softAssertEquals(tcRs.getValue("BLK_CD"), "01825", "Verify that the block code [" + tcRs.getValue("BLK_CD") + "] is that which is expected [01825].");
        ...
        do {
            ...
            if (!tcGstRs.getValue("TC_ID").equals("NULL")) {
                ...
                // TestReporter.softAssertEquals(tcGstRs.getValue("TC_ID"), getBook().getTravelComponentId(), "Verify that the TC ID [" +
                // tcGstRs.getValue("TC_ID") + "] is that which is expected [" + getBook().getTravelComponentId() + "].");
                // TestReporter.softAssertTrue(Regex.match("[0-9]{9}", tcGstRs.getValue("TXN_IDVL_PTY_ID")), "Verify that the transaction individual party ID ["
                // + tcGstRs.getValue("TXN_IDVL_PTY_ID") + "] is a 9-digit number as expected.");
                ...
                // TestReporter.softAssertEquals(tcGstRs.getValue("AGE_NB"), getHouseHold().primaryGuest().getAge(), "Verify that the age [" +
                // tcGstRs.getValue("AGE_NB") + "] is that which is expected [" + getHouseHold().primaryGuest().getAge() + "].");
            } else {
                ...
                // TestReporter.softAssertEquals(tcGstRs.getValue("TC_ID"), "NULL", "Verify that the TC ID [" + tcGstRs.getValue("TC_ID") + "] is that which is
                // expected [NULL].");
                // TestReporter.softAssertEquals(tcGstRs.getValue("TXN_IDVL_PTY_ID"), "NULL", "Verify that the transaction individual party ID [" +
                // tcGstRs.getValue("TXN_IDVL_PTY_ID") + "] is that which is expected [NULL].");
                ...
                // TestReporter.softAssertEquals(tcGstRs.getValue("AGE_NB"), "NULL", "Verify that the age [" + tcGstRs.getValue("AGE_NB") + "] is that which is
                // that which is expected [NULL].");
            }
            tcGstRs.moveNext();
