...
public class TestUpdateComments_parentTC_updateNothing extends AccommodationBaseTest{
...
String parentId = "";
	...
	public void testUpdateComments_parentTC_updateNothing() {
		...
		//Pass the same request, updating nothing
		update.setparentIds(parentId);
		update.setIsActive(isActive);
		update.setSendToGSR(sendToGSR);
		update.setConfidential(confidential);
		...
		TestReporter.logStep("Verify nothing has been updated, and response xml contains none of the nodes");
		TestReporter.setAssertFailed(false);
		TestReporter.logAPI(!update.getResponseStatusCode().equals("200"), "An error occurred getting options by filter", update);
		try {update.getSendToGSR();}catch(XPathNotFoundException e){TestReporter.softAssertTrue(true, "GSR is not present in the xml.");}
		try {update.getConfidential();}catch(XPathNotFoundException e){TestReporter.softAssertTrue(true, "Confidential is not present in the xml.");}
		try {update.getProfileId();}catch(XPathNotFoundException e){TestReporter.softAssertTrue(true, "Profile Id is not present in the xml.");}
		try {update.getProfileCode();}catch(XPathNotFoundException e){TestReporter.softAssertTrue(true, "Profile Code is not present in the xml.");}
		try {update.getCommentId();}catch(XPathNotFoundException e){TestReporter.softAssertTrue(true, "Comment Id is not present in the xml.");}
		try {update.getCommentText();}catch(XPathNotFoundException e){TestReporter.softAssertTrue(true, "Comment Text is not present in the xml.");}
		try {update.getCommentLevel();}catch(XPathNotFoundException e){TestReporter.softAssertTrue(true, "Comment Level is not present in the xml.");}
		try {update.getTcId();}catch(XPathNotFoundException e){TestReporter.softAssertTrue(true, "TC Id is not present in the xml.");}
		try {update.getTpId();}catch(XPathNotFoundException e){TestReporter.softAssertTrue(true, "TP Id is not present in the xml.");}
		try {update.getTpsId();}catch(XPathNotFoundException e){TestReporter.softAssertTrue(true, "TPS Id is not present in the xml.");}
		try {update.getcreatedBy();}catch(XPathNotFoundException e){TestReporter.softAssertTrue(true, "CreatedBy is not present in the xml.");}
		try {update.getcreatedDate();}catch(XPathNotFoundException e){TestReporter.softAssertTrue(true, "CreatedDate is not present in the xml.");}
		try {update.getupdatedBy();}catch(XPathNotFoundException e){TestReporter.softAssertTrue(true, "UpdatedBy Id is not present in the xml.");}
		try {update.getupdatedDate();}catch(XPathNotFoundException e){TestReporter.softAssertTrue(true, "UpdatedDate Id is not present in the xml.");}
		TestReporter.assertAll();
	}
      ...
      //If sendToGsr=true, validate GSR data in EXT_INTF.GSR_RCD, EXT_INTF.GSR_GUEST, and EXT_INTF.GSR_TXN tables
	    if(update.getSendToGSR().equals("true")){
	    	...
	    	TestReporter.setAssertFailed(false);		
	    	TestReporter.softAssertEquals(rs.getValue("TPS_ID"), update.getTpsId() , "Verify that the GSR_RCD tps id [ " + rs.getValue("TPS_ID") + "] matches the comment data [ " + update.getTpsId() + "]");		
	    	TestReporter.softAssertEquals(rs.getValue("CMT_TX"), update.getCommentText() , "Verify that the GSR_RCD comment text [ " + rs.getValue("CMT_TX") + "] matches the comment data [ " + update.getCommentText() + "]");
