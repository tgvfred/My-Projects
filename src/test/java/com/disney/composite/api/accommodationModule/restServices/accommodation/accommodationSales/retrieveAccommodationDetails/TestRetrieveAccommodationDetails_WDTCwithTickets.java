...
public class TestRetrieveAccommodationDetails_WDTCwithTickets extends AccommodationBaseTest {
    ...
    public void testRetrieveAccommodationDetails_WDTCwithTickets() {
        ...
        for (int i = 0; i <= rr.length - 1; i++) {
            TestReporter.softAssertTrue(tp.equals(rr[0].getTravelPlanId().toString()), "The travel plan id in the request [" + tp + "] matches the travel plan in the response [" + rr[0].getTravelPlanId() + "]");
